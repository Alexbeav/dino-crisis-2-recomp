...
#
# Writes: dist/dino-crisis-2-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix dino-crisis-2 \
  --exe-name Dino_Crisis_2_Recompiled \
  --display-name "Dino Crisis 2 Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Dino Crisis 2 (USA).cue" \
